/* eslint-disable jsdoc/check-tag-names, unicorn/numeric-separators-style */
// Generated by scripts/generate.js from KhronosGroup/glTF@c184327.
// Run that script to update; do not edit by hand.
// The glTF 2.0 core specification, from specification/2.0/schema.
...
export type AccessorSparse = GlTFProperty & {
   ...
   * accessor values. The number of indices is equal to `count`. Indices
...
/**
 * An object pointing to a buffer view containing the indices of deviating
 * accessor values. The number of indices is equal to `accessor.sparse.count`.
 ...
 * The elements are tightly packed. Data **MUST** be aligned following the same
 * rules as the base accessor.
 */
...
export type Animation = GlTFChildOfRootProperty & {
  /**
   * An array of animation channels. An animation channel combines an animation
   * sampler with a target property being animated. Different channels of the
   * same animation **MUST NOT** have the same targets.
   *
   * @minItems 1
   */
...
export type AnimationChannelTarget = GlTFProperty & {
   ...
   * axes. For the `"rotation"` property, the values are a quaternion in the
...
/**
 * An animation sampler combines timestamps with a sequence of output values and
...
export type Asset = GlTFProperty & {
  ...
  version: string;
  /**
   * The minimum glTF version in the form of `<major>.<minor>` that this asset
   * targets. This property **MUST NOT** be greater than the asset version.
   */
  minVersion?: string;
};
...
export type CameraOrthographic = GlTFProperty & {
  /**
   * The floating-point horizontal magnification of the view. This value **MUST
   * NOT** be equal to zero. This value **SHOULD NOT** be negative.
   */
  xmag: number;
  /**
   * The floating-point vertical magnification of the view. This value **MUST
   * NOT** be equal to zero. This value **SHOULD NOT** be negative.
   */
  ymag: number;
  /**
   * The floating-point distance to the far clipping plane. This value **MUST
   * NOT** be equal to zero. `zfar` **MUST** be greater than `znear`.
   */
  zfar: number;
...
export type CameraPerspective = GlTFProperty & {
  ...
  aspectRatio?: number;
  /**
   * The floating-point vertical field of view in radians. This value **SHOULD**
   * be less than π.
   */
  yfov: number;
...
export type Image = GlTFChildOfRootProperty & {
  ...
  bufferView?: GlTFId;
    ...
    uri?: string;
    /**
     * The image's media type. This field **MUST** be defined when `bufferView`
     * is defined.
     */
    mimeType?: "image/jpeg" | "image/png" | string;
    /**
...
export type Material = GlTFChildOfRootProperty & {
   ...
   * undefined, all the default values of `pbrMetallicRoughness` **MUST**
   * apply.
   */
...
/**
 * A set of primitives to be rendered. Its global transform is defined by a node
 * that references it.
 */
...
export type MeshPrimitive = GlTFProperty & {
   ...
   * @minItems 1
   *
   * Items: A plain JSON object specifying attributes displacements in a morph target, where each key corresponds to one of the three supported attribute semantic (`POSITION`, `NORMAL`, or `TANGENT`) and each value is the index of the accessor containing the attribute displacements' data.
   */
  targets?: [
    {
      [k: string]: GlTFId;
